...
class TestApiKeysPermission(APITestCase):
    def setUp(self):
        ...
        self.service1_client.credentials(HTTP_AUTHORIZATION=f"Api-Key {APIKey.objects.create_key(name=ApiKeyServicesNames.SERVICE1)[1]}")
        self.service2_client = APIClient()
        self.service2_client.credentials(HTTP_AUTHORIZATION=f"Api-Key {APIKey.objects.create_key(name=ApiKeyServicesNames.SERVICE2+'random_suffix')[1]}")
